...
/**
 * This class is to discover client's IP, drop request if IP has been blacklisted. The blacklisted
 * are in database. Then they are loaded quick list so you reduce hitting db on each request. Then
 * they are checked, comparing IP string and its source. If found incident is reported to IPLOGS db
 * as blocked coded 1.
 * 
 * @author toks
 */
public class RequestIPInterceptor extends HandlerInterceptorAdapter {
  ...
  /**
   * This method should be single purposed, now it does 2 things. Discovers IP and sets request
   * property with 2 different attributes (IP and how it has been discovered). You may create a
   * separate object with these 2 properties (IP and discovered) Perhaps change tp (if else)
   * 
   * @param request
   * @return
   */
  public String getClientIpAddr(HttpServletRequest request) {
    ...
    request.setAttribute(INTERCEPTED_IP_SOURCE, "immediate");
    ...
    if (ip == null || ip.length() == 0 || UNKNOWN_SOURCE.equalsIgnoreCase(ip)) {
      ...
      ip = request.getHeader(X_FORWARDED_FOR_SOURCE);
    }
    return ip;
  }
}
